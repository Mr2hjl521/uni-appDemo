	...
	<view class="mpvue-picker">
		...
		<view class="mpvue-picker-content " :class="{ 'mpvue-picker-view-show': showPicker }">
			...
			>
				<block>
					<picker-view-column>
						<view class="picker-item" v-for="(item, index) in pickerValueSingleArray" :key="index" :class="[item.value==0?'picker-item2':'picker-item']">{{ item.label }}</view>
					</picker-view-column>
				</block>
			</picker-view>
			<!-- 时间选择器 -->
        ...
        methods: {
            initPicker(valueArray) {
                ...
                if (this.mode === 'selector') {
					...
					// 在这里判断
					for(var i=0;i< this.pickerValueSingleArray.length;i++) {
						if(this.pickerValueSingleArray[i].value==0) {
							//未合作的
							this.key=true
						}
