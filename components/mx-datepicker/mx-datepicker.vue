	...
	created() {
		uni.getStorage({
			...
			success: function(res) {
				...
				// 查询服务人员信息
				uni.request({
					url: 'http://192.168.3.10:8082/serviceUserTime/selectServiceUserByUserId', //仅为示例，并非真实接口地址。
					data: {
						serviceUserId: id
					},
					header: {
					...
					success: res => {
						// console.log(res.data.boList); //数组
						var arrs=[]
						// var arr = ['1011', '1013', '1014', '1015'];
						var m = res.data.boList;
						for(var k=0;k<m.length;k++) {
							var str = m[k];
							var arr1 = str.slice(5, 10);
							// console.log(arr1);
							var arr2 = arr1.substring(0, 2);
							var arr3 = arr1.substring(3, 5);
							// console.log(arr2);
							// console.log(arr3);
...
// 						arr.push(arr9);
						...
						// console.log(DateTools.getHoliday.holidays);
					}
				});

				// 			that.Get('http://192.168.3.10:8082/serviceUserTime/selectServiceUserByUserId', { serviceUserId: id }, data => {
				// 				// console.log(data.serviceUserInfoBo); //服务人员基本信息哦
				// 				console.log(data.boList); //服务人员的服务时间哦
				// 				that.serviceStaff=data.serviceUserInfoBo;
				// 				// console.log(that.serviceStaff); //服务人员基本信息哦
				//
				// 			});
			}
		});
