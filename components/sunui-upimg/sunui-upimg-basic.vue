<template name='sunui-upimg'>
	<view>
		<view class="sunsin_picture_list">
			<view v-for="(item,index) in upload_picture_list" :key="index" class="sunsin_picture_itema sunsin_picture_item">
				<image class="sunsin_width" :src="item.path" mode="aspectFill"></image>
		...
		const upload_task = await uni.uploadFile({
			...
			async success(res) {
				if (res.statusCode == 200) {
					...
					console.log(`%c 后端上传(成功返回地址):${data.data}`, 'color:#1AAD19');
					var imgUrl=data.data;
					 uni.$emit('update',{imgUrl:imgUrl})
					// 先获取图片url(各个后端返回值不一，所以造成出入)
					// 修改获取的图片返回值路径
					// 提示：data.info改为你图片返回地址即可
					// upload_picture_list[j]['path_server'] = data.data; //修改返回图片地址即可(2019/05/06) => 无论是腾讯云还是阿里云或者其它(只要返回图片地址以及引入对应的js文件进行一些配置即可扩展本插件)
