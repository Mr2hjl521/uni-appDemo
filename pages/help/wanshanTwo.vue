<template>
	<view class="container">
		...
		<view class="name">
			...
			<input type="text" class="inputBox" v-model="name" placeholder="请务必输入真实姓名" />
		</view>
		<view class="name">
...
export default {
	...
	methods: {
		...
		uImageTap() {
			...
			setTimeout(() => {
				// 判断该填写的都填写了吗?
				if(that.doorNumber==''||that.floorNumber==''||that.schoolId==''||that.name=='') {
					uni.showModal({
						title: '提示',
						...
						uni.getStorage({
							...
							success: function(res) {
								...
								that.Post(
									...
									data => {
										// 贴膜+代取 发现用户没有完善信息到达此页面 保存信息之后页面返回一层
										uni.navigateBack({											
											delta:1										,
										})
									}
						...
						uni.getStorage({
							...
							success: function(res) {
								...
								that.Post(
									'http://39.99.137.77:8081/projects/clientUserAddress/completeMaterialClientUser',
									{
										 "avatarUrl": 'b/2019/12/0_20191206170150.png',
										 "campusId": Number(that.schoolId),
										 "campusName": that.schoolData,
										 "clientUserId": id,
										 "cuId": id,
